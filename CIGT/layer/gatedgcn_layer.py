...
class GatedGCNLayer(pyg_nn.conv.MessagePassing):
    ...
    def __init__(self, in_dim, out_dim, dropout, residual, act='relu',
        ...
        self.geo = pyg_nn.Linear(20, out_dim, bias=True)
    ...
    def forward(self, batch):
        x, e, edge_index, geo = batch.x, batch.edge_attr, batch.edge_index, batch.geo
    ...
    def message(self, Dx_i, Ex_j, PE_i, PE_j, Ce, geo):
        """
        {}x_i           : [n_edges, out_dim]
        {}x_j           : [n_edges, out_dim]
        {}e             : [n_edges, out_dim]
        """
        e_ij = (Dx_i + Ex_j + Ce) * self.geo(geo)
