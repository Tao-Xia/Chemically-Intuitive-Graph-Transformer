...
class GatedGeoGCNLayer(pyg_nn.conv.MessagePassing):
    """
        GatedGCN layer
    ...
    def forward(self, batch):
        ...
        pos             : [n_nodes, 4] (the first column is Z, reset of are 3Dcoor)
        """
        x, e, edge_index, pos = batch.x, batch.edge_attr, batch.edge_index, batch.pos
        if self.residual:
            x_in = x
            e_in = e
        ...
        # bond_length = ((pos[:, 1:])[row] - (pos[:, 1:])[col]).norm(dim=-1).to(batch.x.device)
        # pt = Chem.GetPeriodicTable()
        # atomic_number = pos[:, 0]
        # radii_row = torch.tensor([pt.GetRcovalent(int(z)) for z in atomic_number[row]], device=batch.x.device) # row conv radius
        # radii_col = torch.tensor([pt.GetRcovalent(int(z)) for z in atomic_number[col]], device=batch.x.device) # col conv radius
        # self.rel_bond_length = ((radii_col + radii_row) * 1.2) / bond_length #  
...
@register_layer('gatedgeogcnconv')
class GatedGeoGCNGraphGymLayer(nn.Module):
    """GatedGCN layer.
    Residual Gated Graph ConvNets
    https://arxiv.org/pdf/1711.07553.pdf
    """
    def __init__(self, layer_config: LayerConfig, **kwargs):
        super().__init__()
        self.model = GatedGeoGCNLayer(in_dim=layer_config.dim_in,
