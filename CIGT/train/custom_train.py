...
from CIGT.loss.subtoken_prediction_loss import subtoken_cross_entropy
from CIGT.utils import cfg_to_dict, flatten_dict, make_wandb_name
...
@register_train('vb_test')
def load_model_and_predict(loggers, loaders, model, optimizer=None, scheduler=None):
    """
    Load a pre-trained model and perform predictions.
    ...
    predictions = []
    start_time = time.perf_counter()
    # checkpoint_path = "/home/xiatao/vbnet/GraphGPS-main/results/Final_mse/0/ckpt/232.ckpt"
    if cfg.metric_best == 'mae':
        checkpoint_path = "/home/xiatao/vbnet/GraphGPS-main/results/Final_rel/0/ckpt/915.ckpt"
        # checkpoint_path = "/home/xiatao/vbnet/GraphGPS-main/results/Final_rel_sigmoid/0/ckpt/907.ckpt"

        # checkpoint_path = "/home/xiatao/vbnet/GraphGPS-main/results/Final_log_1e-4/0/ckpt/940.ckpt"
    else:
        checkpoint_path = "/home/xiatao/vbnet/GraphGPS-main/results/Final_binary/0/ckpt/810.ckpt"
    ...
    # n = 0
        for data in loaders:
            ...
            vb_str_i = torch.chunk(data.orb, pred.shape[0], dim=0)

            for i in range(len(pred)):
                # print('1:29 '+' '.join(map(str, vb_str_i[i].tolist())) + ' : ' + str(pred[i].item()))
                logging.info('1:{} '.format(((data.nel-data.nae)//2)[0].item())+'  '.join(map(str, vb_str_i[i].tolist())) + ' : ' + str(pred[i].item()))
        ...
        for data in loaders:
            ...
            for i in range(len(pred)):
                if pred[i] > 0.5:
                    logging.info('1:{} '.format(((data.nel-data.nae)//2)[0].item())+' '.join(map(str, vb_str_i[i].tolist())))
                    # logging.info('1:{} '.format(((data.nel-data.nae)//2)[0].item())+' '.join(map(str, vb_str_i[i].tolist())) + ' : ' + str(pred[i].item()))

    logging.info(f'\nDone predicting!')
    logging.info(f'\ntook: {time.perf_counter() - start_time:.2f}s')
    for logger in loggers:
        logger.close()
...
def log_attn_weights(loggers, loaders, model, optimizer=None, scheduler=None):
    ...
    from CIGT.utils import unbatch, unbatch_edge_index
