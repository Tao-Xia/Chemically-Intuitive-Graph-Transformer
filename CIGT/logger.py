...
import CIGT.metrics_ogb as metrics_ogb
from CIGT.metric_wrapper import MetricWrapper
...
class CustomLogger(Logger):
    ...
    #     true = true.detach().cpu().numpy()  # 确保从 GPU 转移到 CPU 并转换为 NumPy 数组
    #     pred = pred.detach().cpu().numpy()  # 同样处理预测值
    #     reformat = lambda x: round(float(x), cfg.round)
    #     return {
    ...
    def regression(self):
        ...
        true_np = true.detach().cpu().numpy()  # 确保从 GPU 转移到 CPU 并转换为 NumPy 数组
        pred_np = pred.detach().cpu().numpy()  # 同样处理预测值
        
        reformat = lambda x: round(float(x), cfg.round)
        
        # 不再调用 .numpy()
        return {
            'mae': reformat(mean_absolute_error(true_np, pred_np)),
            'r2': reformat(r2_score(true_np, pred_np, multioutput='uniform_average')),
            'spearmanr': reformat(eval_spearmanr(true_np, pred_np)['spearmanr']),
            'mse': reformat(mean_squared_error(true_np, pred_np)),
            'rmse': reformat(root_mean_squared_error(true_np, pred_np)),
        }
