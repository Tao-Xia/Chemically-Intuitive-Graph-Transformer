...
from CIGT.loader.dataset.predict_vb import vb_test
import numpy as np
import torch
...
from CIGT.loader.dataset.PYG_VB import VBData
from CIGT.loader.dataset.aqsol_molecules import AQSOL
from CIGT.loader.dataset.coco_superpixels import COCOSuperpixels
from CIGT.loader.dataset.malnet_tiny import MalNetTiny
from CIGT.loader.dataset.voc_superpixels import VOCSuperpixels
from CIGT.loader.split_generator import (prepare_splits,
                                             set_dataset_splits)
from CIGT.transform.posenc_stats import compute_posenc_stats
from CIGT.transform.task_preprocessing import task_specific_preprocessing
from CIGT.transform.transforms import (pre_transform_in_memory,
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        # add dataset of vb data
        elif pyg_dataset_id == 'VB':
            # dataset = VBData(root=dataset_dir, task='log_1e-4')
            dataset = VBData(root=dataset_dir, task=cfg.dataset.task_type)

        elif pyg_dataset_id =='vb_test':
    
            dataset = vb_test(root = cfg.out_dir, transform=None, pre_transform=None, pre_filter=None)
    ...
    if cfg.train.mode != 'vb_test':
        log_loaded_dataset(dataset, format, name) # log information of dataset
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from CIGT.loader.ogbg_code2_utils import idx2vocab, \
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from CIGT.loader.dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from CIGT.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from CIGT.loader.dataset.peptides_structural import \
